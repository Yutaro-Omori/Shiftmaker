...
DEFAULT_SHIFT_INFO = {"worker": "", "weekday": [], "weekday_inv": 0, "date": [], "date_inv": 0}
...
結果は,キー"worker", "date","date_inv","weekday","weekday_inv", を持つJSONオブジェクトとして返してください。
もし情報が見つからない場合は,対応する値として空文字列 "" を使用してください。
なお，今月は{month}月です。
月に関して直接的に入力がされなかった場合は，上の情報を使ってください。

制約:
- "worker" は従業員名を表す文字列です。名前が提供されなかった場合は空文字列 "" を使用してください。
...
- "weekday" は0から6の整数のリストで,0=月曜, 1=火曜, ..., 6=日曜を表します(例: 月，水，金->[0,2,4],木~土->[4,5,6]このように波線で指定されることもあるので注意してください)。言及がなかった場合は空リストにしてください。
...
"""
    ...
    try:
        ...
        worker = str(extracted_data.get("worker", ""))
        ...
        # 入力された担当者名が文字列であることを確認
        if not isinstance(worker, str):
            logging.warning(f"予期しない担当者名形式が抽出されました。空文字に設定します。")
            worker = ""
        ...
        result = {"worker": worker, "date": date, "date_inv": date_inv, "weekday": weekday, "weekday_inv": weekday_inv}
        # 抽出結果をログに出力
